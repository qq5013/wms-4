...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    [RoutePrefix("api/UserClaimMvos")][Authorize]
    public partial class UserClaimMvosController : ApiController
    {

        IUserClaimMvoApplicationService _userClaimMvoApplicationService = ApplicationContext.Current["UserClaimMvoApplicationService"] as IUserClaimMvoApplicationService;
        ...
        public IEnumerable<UserClaimMvoStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            ...
            {
                states = _userClaimMvoApplicationService.Get(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(),new ApiControllerTypeConverter(), new PropertyTypeResolver())
            ...
            return stateDtos;
          } catch (Exception ex) { var response = UserClaimMvosControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        public long GetCount(string filter = null)
        {
          try
          {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                count = _userClaimMvoApplicationService.GetCount(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(),new ApiControllerTypeConverter(), new PropertyTypeResolver()));
            }
            else 
            {
                count = _userClaimMvoApplicationService.GetCount(UserClaimMvosControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs()));
            }
            return count;
          } catch (Exception ex) { var response = UserClaimMvosControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        public void Put(string id, [FromBody]CreateUserClaimMvoDto value)
        {
          try {
            UserClaimMvosControllerUtils.SetNullIdOrThrowOnInconsistentIds(id, value);
            _userClaimMvoApplicationService.When(value as ICreateUserClaimMvo);
          } catch (Exception ex) { var response = UserClaimMvosControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }

        [HttpPatch][SetRequesterId]
        public void Patch(string id, [FromBody]MergePatchUserClaimMvoDto value)
        {
          try {
            UserClaimMvosControllerUtils.SetNullIdOrThrowOnInconsistentIds(id, value);
            _userClaimMvoApplicationService.When(value as IMergePatchUserClaimMvo);
          } catch (Exception ex) { var response = UserClaimMvosControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        {
          try {
            var value = new DeleteUserClaimMvoDto();
            value.CommandId = commandId;
            value.RequesterId = requesterId;
            value.UserVersion = (long)Convert.ChangeType(version, typeof(long));
            UserClaimMvosControllerUtils.SetNullIdOrThrowOnInconsistentIds(id, value);
            _userClaimMvoApplicationService.When(value as IDeleteUserClaimMvo);
          } catch (Exception ex) { var response = UserClaimMvosControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        public IEnumerable<PropertyMetadata> GetMetadataFilteringFields()
        {
          try {
            ...
            foreach (var p in UserClaimMvoMetadata.Instance.Properties)
            {
        ...
        [HttpGet]
        public IUserClaimMvoStateEvent GetStateEvent(string id, long version)
        {
          try {
            var idObj = UserClaimMvosControllerUtils.ParseIdString(id);
            return _userClaimMvoApplicationService.GetStateEvent(idObj, version);
          } catch (Exception ex) { var response = UserClaimMvosControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        private class PropertyTypeResolver : IPropertyTypeResolver
        {
            ...
            {
                return UserClaimMvosControllerUtils.GetFilterPropertyType(propertyName);
            }
        }
    ...
    public static class UserClaimMvosControllerUtils
    {
        ...
        public static void SetNullIdOrThrowOnInconsistentIds(string id, CreateOrMergePatchOrDeleteUserClaimMvoDto value)
        {
            ...
            {
                throw DomainError.Named("inconsistentId", "Argument Id {0} NOT equals body Id {1}", id, value.UserClaimId);
            }
        }
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (UserClaimMvoMetadata.Instance.PropertyMetadataDictionary.ContainsKey(fieldName))
            {
        ...
        {
            if (UserClaimMvoMetadata.Instance.PropertyMetadataDictionary.ContainsKey(propertyName))
            {
                return UserClaimMvoMetadata.Instance.PropertyMetadataDictionary[propertyName].Type;
            }
