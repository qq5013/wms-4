    ...
    public partial class WarehousesController : ApiController
    {
        ...
        public IEnumerable<WarehouseStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            ...
            {
                states = _warehouseApplicationService.Get(GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs())
                    , GetQueryOrders(sort), firstResult, maxResults);
            }
            var stateDtos = new List<WarehouseStateDto>();
            foreach (var s in states)
            {
                var dto = new WarehouseStateDto((WarehouseState)s);
        ...
        public long GetCount(string filter = null)
        {
          try
          {
            ...
            {
                count = _warehouseApplicationService.GetCount(GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs()));
            }
            return count;
          } catch (Exception ex) { var response = GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        protected static void SetNullIdOrThrowOnInconsistentIds(string id, CreateOrMergePatchOrDeleteWarehouseDto value)
        {
            var idObj = id;
