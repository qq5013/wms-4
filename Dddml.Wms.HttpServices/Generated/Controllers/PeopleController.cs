...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    public partial class PeopleController : ApiController
    {
        ...
        IPersonApplicationService _personApplicationService = ApplicationContext.Current["PersonApplicationService"] as IPersonApplicationService;
    ...
    public static class PeopleControllerUtils
    {
        ...
        public static void SetNullIdOrThrowOnInconsistentIds(string id, CreateOrMergePatchOrDeletePersonDto value)
        {
            var idObj = ParseIdString(id.IsNormalized() ? id : id.Normalize());
            if (value.PersonalName == null)
            {
                value.PersonalName = new PersonalNameDto(idObj);
            }
            else if (!((ICreateOrMergePatchOrDeletePerson)value).PersonalName.Equals(idObj))
            {
                throw DomainError.Named("inconsistentId", "Argument Id {0} NOT equals body Id {1}", id, value.PersonalName);
            }
        }
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (PersonMetadata.Instance.PropertyMetadataDictionary.ContainsKey(fieldName))
            {
        ...
        {
            if (PersonMetadata.Instance.PropertyMetadataDictionary.ContainsKey(propertyName))
            {
                return PersonMetadata.Instance.PropertyMetadataDictionary[propertyName].Type;
            }
        ...
        public static IEnumerable<PersonStateDto> ToPersonStateDtoCollection(IEnumerable<PersonalName> ids)
        {
            var states = new List<PersonStateDto>();
            foreach (var id in ids)
            {
                var dto = new PersonStateDto();
                dto.PersonalName = new PersonalNameDto(id);
                states.Add(dto);
            }
            return states;
        }
